...
library(reshape2)
library(tidyverse)
library(coda);library(rjags);library(runjags)
```
...
for(i in 1:K){
    ...
    for(t in 2:(TTT[i]+2)){ #TTT = 5 #number of years regardless of observed or not.
...
data = merge(res2, res1, by='PolicyNum')
final_data2 = subset(data, select = c('PolicyNum', 'n_2006', 'n_2007', 'n_2008', 'n_2009', 'n_2010', 'n_2011',
                                     'TypeCity', 'TypeCounty', 'TypeSchool', 'TypeTown', 'TypeVillage',
                                     'col.Cov.Idx'
))
